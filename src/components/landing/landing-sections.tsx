...
/** The authenticated app's primary button, verbatim (onboarding panels,
 *  timeline disclosures) — the landing page uses the same classes so the
 *  first click feels identical to every click after sign-in. */
export const PRIMARY_BUTTON_CLASSES =
  "rounded-xl bg-zinc-900 px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-zinc-700";

/** The app's quiet bordered button (settings' secondary actions), sized to
 *  sit beside the primary. */
export const SECONDARY_BUTTON_CLASSES =
  "rounded-xl border border-[#e5e5e5] bg-white px-6 py-3 text-sm font-medium text-[#333333] transition-colors hover:border-[#111] hover:text-[#111]";
...
function ChapterHeading({ label, title }: { label: string; title?: string }) {
  return (
    <div>
      ...
      {title ? (
        <h2 className="font-voice mt-2 max-w-[24em] text-[24px] font-medium leading-[1.3] tracking-[-0.3px] text-ink-primary">
          {title}
        </h2>
      ) : null}
    </div>
...
export function LandingHero() {
  return (
    <section className="grid items-center gap-12 lg:grid-cols-[1fr_minmax(0,440px)] lg:gap-16">
      <div>
        <h1 className="font-voice text-[30px] font-medium leading-[1.25] tracking-[-0.02em] text-ink-primary sm:text-[40px] sm:leading-[1.2]">
          Every decision changes who you&apos;re becoming.
        </h1>
        <p className="mt-4 max-w-[32em] text-[15px] leading-[1.7] text-ink-secondary">
      ...
      <div className="w-full max-w-[460px]">
        <p className="text-label text-[var(--accent-futures)]">Future Forecast</p>
        <div className="mt-2">
          <CurrentForecastFutureCard future={LANDING_FORECAST_OPPORTUNITY} />
        </div>
        <p className="mt-3 text-[12px] text-ink-tertiary">
          Real product UI — an example forecast, not your data.
        </p>
      </div>
    </section>
  );
      ...
      <div className="mt-7 flex flex-col">
        {HOW_IT_WORKS.map((step, index) => (
          <div key={step.title} className="flex flex-col">
            ...
            <OverviewCard className="px-7 py-6 text-center">
              <p className="text-label text-ink-tertiary">Step {index + 1}</p>
              <h3 className="font-voice mt-2 text-[20px] font-medium leading-[1.3] text-ink-primary">
                {step.title}
              </h3>
              <p className="mt-2 text-body-small text-ink-secondary">{step.detail}</p>
            </OverviewCard>
          </div>
        ))}
      </div>
  ...
  return (
    ...
    >
      <div className="px-7 py-5">
        ...
        <h4 className="font-voice text-[18px] font-medium leading-[1.3] tracking-[-0.01em] text-ink-primary">
          {path.title}
        </h4>
        ...
        <p className="mt-2 text-body-small text-ink-secondary">{path.summary}</p>
      </div>
    </CardShell>
  );
}
  ...
  const steps: { label: string; accentVar: string; content: React.ReactNode }[] = [
    {
      ...
      content: (
        <OverviewCard className="px-7 py-6">
          <h4 className="font-voice text-[22px] font-medium leading-[1.3] tracking-[-0.3px] text-ink-primary">
            {DEMO_SITUATION.title}
          </h4>
          <p className="mt-3 text-body-small leading-[1.7] text-ink-secondary">
          ...
          <div className="mt-4 flex flex-col gap-4">
            {DEMO_FORECAST.timeline.map((entry) => (
              ...
              >
                ...
                <p className="mt-1 text-body-small text-ink-secondary">{entry.text}</p>
              </div>
            ))}
          </div>
        </OverviewCard>
...
function ExploreFeature({
  ...
  return (
    <div>
      <h3 className="font-voice text-[20px] font-medium leading-[1.3] text-ink-primary">
        {name}
      </h3>
      <p className="mt-1.5 text-[14px] leading-[1.7] text-ink-secondary">{sentence}</p>
  ...
  return (
    <section className="mx-auto w-full max-w-[880px]">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {PRICING.map((plan) => (
          <OverviewCard key={plan.name} className="flex flex-col px-7 py-6">
            <p className="text-[15px] font-semibold text-ink-primary">{plan.name}</p>
            <p className="font-voice mt-2 text-[26px] font-medium text-ink-primary">
              {plan.price}
              {plan.period ? (
                <span className="text-[14px] font-normal text-ink-tertiary">
                  {plan.period}
                </span>
              ) : null}
            </p>
            <ul className="mt-3 flex-1">
              {plan.lines.map((line) => (
                <li
                  key={line}
                  className="flex items-baseline gap-2.5 text-[14px] leading-[1.85] text-[#777777]"
                >
...
export function FinalCtaSection() {
  return (
    <section className="text-center">
      <h2 className="font-voice mx-auto max-w-[18em] text-[26px] font-medium leading-[1.3] tracking-[-0.3px] text-ink-primary sm:text-[32px]">
